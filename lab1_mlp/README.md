...
### 调整dropout
将MLP内部的dropout值调整为0.1，整体的训练曲线更加平滑，模型准确率没有太大变化，最终仍然在97%左右。  
将MLP内部的dropout值调整为0.5，模型拟合能力明显变弱。每个epoch增长的准确率较少，10个epoch后，准确率也才刚刚突破96%。

<center>
<img src="../res/mlp_dropout.png" width="600">
</center>
...
### 最好的模型
将调整方法进行整合。设置`dropout=0.2`，同上一样改变模型的网络结构，同时调整优化器参数`learning rate=0.02`和`momentum=0.6`。最终模型准确率达到98%。
```python
...
- 合适的dropout可以有效正则化模型，降低过拟合的风险。但是dropout过大会导致模型学习能力下降，学习速度变慢。
